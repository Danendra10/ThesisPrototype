...
void CountRobotTarget(uint8_t robot_num, float &target_x, float &target_y)
{
    ...
    for (int i = 0; i <= X_FIELD; i += stepSize)
    {
        for (int j = 0; j <= Y_FIELD; j += stepSize)
        {
            ...
            {
                // when the current
                // if (sqrt((x_curr - enemy[i].pose_x) * (x_curr - enemy[i].pose_x) + (y_curr - enemy[i].pose_y) * (y_curr - enemy[i].pose_y)) < exclude_radius_around_obstacles ||
                //     CheckLineCircleIntersection2(x_curr, y_curr, curr_pose_x, curr_pose_y, enemy[i].pose_x, enemy[i].pose_y, exclude_radius_around_obstacles))
                // {
