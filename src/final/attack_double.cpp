...
void CountTarget()
{
    if (robot[0].is_holding_ball)
    {
        attacker_robot_target.x = X_FIELD;
        attacker_robot_target.y = Y_FIELD_1_2;
    }
    else
    {
...
void MoveRobot(int num)
{
    ...
    logger_instance.Log(logger::RED, "Robot Position[%d]: %f %f || target: %f %f", num, robot[num].pose_x, robot[num].pose_y, target_x, target_y);
